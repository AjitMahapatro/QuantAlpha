...
def create_ticker_feature_frame(
    ...
    frame["volatility_20"] = frame["daily_return"].rolling(20).std() * np.sqrt(252)  # Annualized
    ...
    # Normalize position within the bands (-0.5 to 0.5 is typical)
    frame["bollinger_position"] = (frame["close"] - bollinger_mid) / spread

    # --- Volume and Market Features ---
    ...
    # Align the pre-processed macro data with the ticker's trading days
    ...
    # Note: These introduce look-ahead bias and must be handled carefully during training.
    # The model will predict the 'target_direction' for the *next* day.
    frame["target_direction"] = (frame["close"].shift(-1) > frame["close"]).astype(int)
    frame["target_next_return"] = frame["daily_return"].shift(-1)
    # Example of a secondary target: predicting a change in volatility regime.
